...
void printSequence(const vi &a, const vi &prev, const int &index)
{
    if (index == -1)
    {
        return;
    }
    printSequence(a, prev, prev[index]);
    cout << a[index] << " ";
}
...
int32_t main()
{
    ...
    int lengthLIS(*max_element(all(dp)));
    int index(max_element(all(dp)) - dp.begin());
    // Lenght of LIS
    cout << lengthLIS << endl;
    // Print the LIS
    printSequence(a, prev, index);
